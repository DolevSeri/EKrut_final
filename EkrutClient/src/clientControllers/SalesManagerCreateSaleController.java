...
/**@author peleg
 * a class that will do the functionality of create sale screen of Sales Manager
 * */
public class SalesManagerCreateSaleController {
    ...
    void ClickOnSendToAcrivate(ActionEvent event) {
    	if(cmbArea.getValue()==null || cmbSale.getValue() ==null) {
    		...
    		lblError.setVisible(false);
    		sale=new Sale(Integer.valueOf(txtpatternID.getText()),txtDiscuntType.getText(),txtStartDay.getText(),txtEndDay.getText(),txtStartHour.getText(),txtDuration.getText()
    				,Region.valueOf(cmbArea.getValue()),ChatClient.salesController.getSales().size()+1,SaleStatus.valueOf("NEEDTOACTIVATE"));
    ...
    void chooseSalePatternComboBox(ActionEvent event) {
    	...
    	for(SalesPattern sale:ChatClient.salesPatternController.getSalespattern()) {
    		if(String.valueOf(sale.getPatternID()).equals(id)) {
    		     ...
    		     txtDuration.setText(sale.getDuration());
     ...
     * @param bol-will be true if we want that the labels be visible and false for not visible
     * */
